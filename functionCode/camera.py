...
def get_devices_list():
    devices_list = MV_CC_DEVICE_INFO_LIST()
    # tlayerType = MV_GIGE_DEVICE
    # ret = MvCamera.MV_CC_EnumDevices(tlayerType, devices_list)
    # if ret != 0:
    #     print("enum devices fail! ret[0x%x]" % ret)
    #     sys.exit()
    # if devices_list.nDeviceNum == 0:
    #     print("find no device!")
    #     sys.exit()
    # print("Find %d devices!" % devices_list.nDeviceNum)
    return devices_list


class Camera():
    def __init__(self,device_list_):
        self.cam = MvCamera()
        self.deviceList = device_list_
    ...
    def activatecamera(self,nSelCamIndex):
        ...
        # if ret != 0:
        #     print('show error', 'turn off triggermode fail ' + self.To_hex_str(ret))
        #     return ret
        stBool = c_bool(False)
