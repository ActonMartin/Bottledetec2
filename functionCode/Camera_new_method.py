...
def image_control(data, stFrameInfo):
    if stFrameInfo.enPixelType == 17301505:
        ...
        numArray = np.zeros([stFrameInfo.nHeight, stFrameInfo.nWidth, 1], "uint8")
        numArray[:, :, 0] = image
        image = numArray
    ...
    if active_way == "getImagebuffer":
        ...
        for i in range(5):
            ...
            if None != stOutFrame.pBufAddr and 0 == ret and stOutFrame.stFrameInfo.enPixelType == 17301505:
                ...
                numArray = Mono_numpy(data, stOutFrame.stFrameInfo.nWidth,
        ...
        #         image_ = image_control(data=image, stFrameInfo=stFrameInfo)
        #         print(type(image_))
        #     else:
        #         print("no data[0x%x]" % ret)
        while True:
            ...
            if ret == 0:
                ...
                if PixelType_Gvsp_Mono8 == stFrameInfo.enPixelType:
                    numArray = Mono_numpy(pData, stFrameInfo.nWidth,
                                          stFrameInfo.nHeight)
                return numArray
            else:
                # print("no data[0x%x]" % ret)
...
def main():
    ...
    cam1, stDeviceList1 = creat_camera(deviceList, 1, log=False)
    cam2, stDeviceList2 = creat_camera(deviceList, 2, log=False)
    cam3, stDeviceList3 = creat_camera(deviceList, 3, log=False)
    cam4, stDeviceList4 = creat_camera(deviceList, 4, log=False)
    # cam5, stDeviceList5 = creat_camera(deviceList, 5, log=False)
    ...
    start_grab_and_get_data_size(cam3)
    start_grab_and_get_data_size(cam4)
    # start_grab_and_get_data_size(cam5)

    kjh0 = access_get_image(cam,"getoneframetimeout")
    kjh1 = access_get_image(cam1,"getoneframetimeout")
    kjh2 = access_get_image(cam2,"getoneframetimeout")
    kjh3 = access_get_image(cam3,"getoneframetimeout")
    kjh4 = access_get_image(cam4,"getoneframetimeout")
    if kjh0 is not None:
        print('kjh0',kjh0.shape)
    if kjh1 is not None:
        print('kjh1',kjh1.shape)
    if kjh2 is not None:
        print('kjh2',kjh2.shape)
    if kjh3 is not None:
        print('kjh3',kjh3.shape)
    if kjh4 is not None:
        print('kjh4',kjh4.shape)

    # cv2.imshow('kk',kjh)
    # cv2.waitKey(0)
    # access_get_image(cam,"getImagebuffer")
    # # 设置缓存节点个数
