...
from PyQt5.QtCore import QSettings, QTimer, Qt, QDateTime,QSize
from PyQt5.QtGui import QIcon, QImage, QPixmap,QCloseEvent

from PyQt5.QtWidgets import (QApplication, QMainWindow,QComboBox,QMessageBox,QLabel)
...
class MainWindows(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.pushButton_2.clicked.connect(self.load_camera_old)
        # self.ui.loadcamera_pushButton.clicked.connect(self.load_camera_old)
        self.ui.pushButton_3.clicked.connect(self.load_timer)
        ...
        # self.time_count = 0
        # self.save_pic('00')
        self.cam1_emit_signal = False
        self.cam2_emit_signal = False
        self.cam3_emit_signal = False
        self.cam4_emit_signal = False
        self.cam5_emit_signal = False
        self.cam6_emit_signal = False
    ...
    def hide1(self): # 隐藏部分按钮+label
        self.ui.groupBox_0.hide()
    ...
    def emit_singal_part1(self,list_,a):
        # todo 对六个相机进行发送信号测试
        if list_:
            if a == "CAMERA_1":
                self.cam1_emit_signal = True
            if a == "CAMERA_2":
                self.cam2_emit_signal = True
            if a == "CAMERA_3":
                self.cam3_emit_signal = True
            if a == "CAMERA_4":
                self.cam4_emit_signal = True
            if a == "CAMERA_5":
                self.cam5_emit_signal = True
            if a == "CAMERA_6":
                self.cam6_emit_signal = True

    def emit_signal_part2(self):
        if self.cam1_emit_signal:
            try:
        ...
        if self.cam2_emit_signal and self.cam4_emit_signal:
            try:
                self.dll_instance.serial_port_Beep_ON('09')
                self.cam2_emit_signal = False
                self.cam4_emit_signal = False
            except:
                pass

        if self.cam2_emit_signal:
            try:
        ...
        if self.cam4_emit_signal:
            try:
                self.dll_instance.serial_port_Beep_ON('08')
                self.cam4_emit_signal = False
            except:
                pass

        if self.cam3_emit_signal:
            try:
        ...
        if self.cam5_emit_signal:
            try:
                self.dll_instance.serial_port_Beep_ON('10')
                self.cam5_emit_signal = False
            except:
                pass

        if self.cam6_emit_signal:
            try:
                self.dll_instance.serial_port_Beep_ON('20')
                self.cam6_emit_signal = False
            except:
                pass
    ...
    def make_easy(self, label_id, cam_id,a):
        ...
        if image_from_camera is not None:
            ...
            self.emit_singal_part1(defect_list,a)
            self.showresult()
    ...
    def change_image_size_on_label(self, image):
        ...
        if int(self.windowState()) == 2:
            image = cv2.resize(image, (int(600), int(340)))
