    ...
    def show_image_on_label(self):
        if self.cam1_flag:
            ...
            try:
                ...
                self.cam1_check_flag = 1
            except:
                command = '66 AA FF FF 01 00 FF BB'
                command_ = bytes.fromhex(command)
                self.serial.write(command_)
                print('cam1 error')
                self.cam1_check_flag = 0
        ...
        if self.cam2_flag:
            ...
            try:
                ...
                self.cam2_check_flag = 1
            except:
                command = '66 AA FF FF 02 00 FF BB'
                command_ = bytes.fromhex(command)
                self.serial.write(command_)
                print('cam2 error')
                self.cam2_check_flag = 0
        ...
        if self.cam3_flag:
            ...
            try:
                ...
                self.cam3_check_flag = 1
            except:
                command = '66 AA FF FF 03 00 FF BB'
                command_ = bytes.fromhex(command)
                self.serial.write(command_)
                print('cam3 error')
                self.cam3_check_flag = 0
        ...
        if self.cam4_flag:
            ...
            try:
                ...
                self.cam4_check_flag = 1
            except:
                command = '66 AA FF FF 04 00 FF BB'
                command_ = bytes.fromhex(command)
                self.serial.write(command_)
                print('cam4 error')
                self.cam4_check_flag = 0
        ...
        if self.cam5_flag:
            ...
            try:
                ...
                self.cam5_check_flag = 1
            except:
                command = '66 AA FF FF 05 00 FF BB'
                command_ = bytes.fromhex(command)
                self.serial.write(command_)
                print('cam5 error')
                self.cam5_check_flag = 0
        ...
        if self.cam6_flag:
            ...
            try:
                ...
                self.cam6_check_flag = 1
            except:
                command = '66 AA FF FF 06 00 FF BB'
                command_ = bytes.fromhex(command)
                self.serial.write(command_)
                print('cam6 error')
                self.cam5_check_flag = 0

        def check_6_cam(flag):
            if flag:
                if self.cam1_check_flag and self.cam2_check_flag and self.cam3_check_flag and self.cam4_check_flag \
                     and self.cam5_check_flag and self.cam6_check_flag:
                    command = '66 AA FF FF 00 00 FF BB'
                    command_ = bytes.fromhex(command)
                    self.serial.write(command_)
                    time.sleep(0.2)
                    self.a_check_flag = False

        check_6_cam(self.a_check_flag)
